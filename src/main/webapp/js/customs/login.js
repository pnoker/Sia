var loginForm = Ext.create('Ext.form.FormPanel', {
	...
	items: [{
		...
		html: ['<img id="validate" src="/Sia/validate.do" style="width:100%;height:40px">',
...
function login() {
	if (loginForm.form.isValid()) {
		Ext.Ajax.request({
			method: 'get',
			url: ctxPath + '/loginIn.do',
			params: {
				username: encodeURI(Ext.String.trim(loginForm.getForm().findField('username').getValue())),
				password: encodeURI(Ext.String.trim(loginForm.getForm().findField('password').getValue())),
				validate: encodeURI(Ext.String.trim(loginForm.getForm().findField('validate').getValue())),
			...
			success: function (response) {
				...
				if (responseText.result != 'Y') {
					...
					document.location.href = ctxPath + '/index.do';
				}
			}
		});
	}
...
function reLoadValidate() {
	...
	validate.src = '/Sia/validate.do?rnd=' + Math.random();
...
function QRlogin() {
	...
	Ext.Msg.alert({
		title: '二维码登陆',
		message: [
			'<img src="/Sia/qrImage.do?rnd=' + rnd + '" width="250px" height="250px">',
